package dev.zypec.particles.effect.script.particle.reader;
...
import dev.zypec.particles.effect.script.argument.type.ItemStackArgument;
...
import dev.zypec.particles.effect.script.particle.config.Billboard;
...
public abstract class ParticleReader<T extends ParticleSpawner> extends ScriptReader<ParticleReader.Context<T>, T> {
    ...
    public ParticleReader() {
        ...
        addValidArgument(c -> {
            ...
            if (item == null) return;
            c.script().particleData(PacketHandler.NMS.getGenericData(particle, item));
        }, "item");
        ...
        addValidArgument(c -> {
            ...
            var item = ItemStackArgument.read(c);
            if (item == null) return;
            c.script().particleData(PacketHandler.NMS.getGenericData(particle, Bukkit.createBlockData(item.getType())));
        }, "block");
        ...
        addValidArgument(c -> {
            ...
            try {
                c.script().particleData(PacketHandler.NMS.getGenericData(particle, StaticArgument.asInt(c)));
        ...
        addValidArgument(c -> {
            ...
            try {
                c.script().particleData(PacketHandler.NMS.getGenericData(particle, StaticArgument.asFloat(c)));
            } catch (ReaderException e) {
                error(c, "Unexpected '" + c.key() + "' value: " + c.value(), e.getMessage());
            } catch (Exception ignored) {
                error(c, "Unexpected '" + c.key() + "' value: " + c.value());
