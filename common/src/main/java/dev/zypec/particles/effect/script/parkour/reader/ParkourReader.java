package dev.zypec.particles.effect.script.parkour.reader;
...
import dev.zypec.particles.effect.script.parkour.Parkour;
...
public class ParkourReader extends ScriptReader<ParkourReader.Context, Parkour> {
    ...
    public ParkourReader() {
        ...
        addValidArgument(c -> {
            var script = Particles.getEffectManager().getPresets().read(c.effect(), c.value());
        ...
        addValidArgument(c -> c.script().configure(Particles.getEffectManager().read(c.effect(), "preset", c.value())), "configure");
        addValidArgument(c -> c.script().whenSpawned(Particles.getEffectManager().read(c.effect(), "preset", c.value())), "when-spawned");
        addValidArgument(c -> c.script().whenSucceeded(Particles.getEffectManager().read(c.effect(), "preset", c.value())), "when-succeeded");
        addValidArgument(c -> c.script().whenFailed(Particles.getEffectManager().read(c.effect(), "preset", c.value())), "when-failed");
        addValidArgument(c -> c.script().whenStarted(Particles.getEffectManager().read(c.effect(), "preset", c.value())), "when-started");
