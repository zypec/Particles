...
import dev.zypec.particles.color.data.RandomNoteColorData;
import dev.zypec.particles.color.data.duo.DuoImpl;
...
import dev.zypec.particles.effect.script.argument.ScriptArgument;
import dev.zypec.particles.effect.script.argument.type.FloatArgument;
...
import dev.zypec.particles.util.math.Vectors;
...
import dev.zypec.particles.util.nms.particles.ParticleEffect.Property;
...
public class ParticleSpawner extends Script {
    ...
    public ParticleContext tick(EffectData data, HandlerEvent event, boolean configureOffset, boolean rotatePos) {
        ...
        // Viewers
        builder.viewers(Particles.getPlayerManager().defaultFilter(player));
    ...
    public void updateParticleData(ParticleBuilder builder, EffectData data) {
        if (colorData == null && particleData != null) {
            builder.data(particleData instanceof ScriptArgument<?> argument ? PacketHandler.NMS.getGenericData(particle, argument.get(this, data)) : particleData);
            return;
        }

        if (colorData == null) {
            particleData = PacketHandler.NMS.getParticleParam(particle);
        ...
        if (particle.hasProperty(Property.PARAM_DUST)) {
            ...
            if (particle == ParticleEffect.DUST_COLOR_TRANSITION)
                if (colorData instanceof DuoImpl duo) {
                    ...
                    builder.data(PacketHandler.NMS.getDustTransitionData(pair.getKey(), pair.getValue(), size));
                } else
                    builder.data(PacketHandler.NMS.getDustTransitionData(colorData.next(data), colorData.tempNext(data), size));
            else
                builder.data(PacketHandler.NMS.getDustData(colorData.next(data), size));
        } else if (particle.hasProperty(Property.PARAM_SPELL)) {
            builder.data(PacketHandler.NMS.getSpellData(particle, colorData.next(data), colorAlpha, particleData instanceof FloatArgument power ? power.get(this, data) : 1F));
        } else if (particle.hasProperty(Property.PARAM_COLOR)) {
            builder.data(PacketHandler.NMS.getColorData(particle, colorData.next(data), colorAlpha));
        } else if (particle.hasProperty(Property.OFFSET_COLOR)) {
            builder.data(PacketHandler.NMS.getParticleParam(particle));
