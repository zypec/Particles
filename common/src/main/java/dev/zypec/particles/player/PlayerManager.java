package dev.zypec.particles.player;
...
public class PlayerManager {
    ...
    public void initializePlayer(Player player, Consumer<PlayerEffectData> callback) {
        Bukkit.getScheduler().runTaskAsynchronously(Particles.getPlugin(), () -> {
            var effectManager = Particles.getEffectManager();
            var colorManager = Particles.getColorManager();
            var database = Particles.getDatabase();
    ...
    public void remove(Player player) {
        ...
        Bukkit.getScheduler().runTaskAsynchronously(Particles.getPlugin(), () -> save(player, data));
    }
    ...
    public void save(Player player, PlayerEffectData data) {
        ...
        Particles.getDatabase().update("REPLACE INTO `" + DatabaseManager.TABLE + "` (`uuid`, `data`) VALUES (?, ?)", player.getUniqueId().toString(), gson.toJson(playerData));
    }

    private void startTask() {
        var autoSaveInterval = Particles.getConfig().getInt("auto-save-interval", 10);
        if (autoSaveInterval <= 0) {
            taskId = -5;
            return;
        }
        taskId = Bukkit.getScheduler().runTaskTimerAsynchronously(Particles.getPlugin(), () -> Bukkit.getOnlinePlayers().forEach(player -> save(player, getEffectData(player.getUniqueId()))), 20L * 60 * autoSaveInterval, 20L * 60 * autoSaveInterval).getTaskId();
    }
